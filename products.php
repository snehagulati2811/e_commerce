...
if (!empty($_POST['keyword'])) {
    ...
    $sqlSelectSearchProduct = mysqli_query($con, "select * from product_detail where name like '$productName%'") or die(mysql_error());
    if (mysqli_num_rows($sqlSelectSearchProduct) >= 1) {
        while ($getRowSearchProduct = mysqli_fetch_array($sqlSelectSearchProduct)) {
            //$prodNo = $getRowSearchProduct["prod_no"];
            $prodId = $getRowSearchProduct["Product_id"];
            $prodName = $getRowSearchProduct["name"];
            $prodPrice = $getRowSearchProduct["Price"];
            $prod_img = $getRowSearchProduct["Product_image"];

            $displayProdCat .= '<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 product_gallery" style="">
...
<html>
    <head>
        ...
        <title>E-Coffee Shop - Product</title>
    </head>
