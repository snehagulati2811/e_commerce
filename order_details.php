...
$no = $_GET['orderno'];
$alldetails = mysqli_query($con, "SELECT * FROM order_details od INNER JOIN product_detail pd ON od.product_id = pd.Product_id WHERE od.order_no = '$no' ") or die(mysql_error());
?>
...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>E-Coffee Shop - Order Details</title>
                ...
                <body style="background-color:white">
                    ...
                    <div class="container-fluid">
                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <center><h2>Order Info<h2>                                        <a style="color:white" class="btn btn-primary btn-mini" href="my_orders.php">My Orders</a></center>

                                        <form name="editdelete_product_form" method="post" action="admin_addnew_product.php" enctype="multipart/form-data">
                                            <div></div>
                                            ...
                                            <table style="width:100%;border: 1px;border-color: white;background-color: white;" align="left" class="table table-hover">
                                                ...
                                                <?php
                                                $i = 0;
                                                while ($orderdetail = mysqli_fetch_array($alldetails)) {
                                                    $orderno = $orderdetail["order_no"];
                                                    $prodname = $orderdetail["name"];
                                                    $prodprice = $orderdetail["Price"];
                                                    $prodquantity = $orderdetail["sold_quantity"];
                                                    $prodship = $orderdetail["shippingCharge"];
                                                    $subttot = ($prodprice * $prodquantity) + $prodship
                                                    ?>		
                                                    <tr>
                                                        ...
                                                        <td><?php echo $prodship; ?></td>
                                                        <td>$<?php echo $subttot; ?></td>
                                                    </tr><?php } ?>
                                            </table> 
                                        </form>
                                        </div>
